...
<CardGroup cols={2}>
  ...
  <Card title="Referral" icon="user-plus">
    `ref_` links set a pending referrer for onboarding.
  </Card>
</CardGroup>
...
All token-based deep links use the same chain codes, address encoding, and optional affiliate/group tail.
...
Use `ref_` links when you only want to set a pending referrer by Telegram username.

```text
ref_{username}
```

Referral links are consumed during onboarding. Token links with affiliate metadata use the same permanent referrer concept: whichever valid referral source reaches a new user first can become their referrer.
